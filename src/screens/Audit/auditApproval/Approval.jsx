...
function Approval() {
  ...
  const handleRecheck = () => {
    entryApis
      .entryCheck({
        data: {
          auditentry_id: (state?.id ?? "").toString(),
          status: "0",
      ...
      .then((data) => {
        ...
        messageToast({
          message: "Record send recheck successfully",
          status,
          title: "Recheck",
        });
        navigate("/auditApproval");
  ...
  return (
    <>
              ...
              <div className="d-flex align-items-right justify-content-between w-100 gap-3 pt-3 ">
                ...
                <div>
                  ...
                  <button
                    className="btn"
                    onClick={() =>
                      setRecheckModal({ ...recheckModal, show: true })
                    }
                    style={{
                      color: "white",
      ...
      {recheckModal?.show && (
        <Modal
          title="Re-Check"
          open={recheckModal?.show}
          onOk={handleRecheck}
          onCancel={() => setRecheckModal({ ...recheckModal, show: false })}>
          <h5>Add Remarks:</h5>
          <TextArea
            rows={4}
            style={{ resize: "none" }}
